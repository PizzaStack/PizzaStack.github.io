...
```
Afterwards, register this script as a `chron` job:
```bash
* * * * * /path/to/script/build.sh
```

This will run the script every minute, but look into changing `chrontab` syntax to change intervals. The script should be made executable, and the server should have all necessary tools installed.
